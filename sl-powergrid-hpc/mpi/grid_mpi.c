...
int main(int argc, char **argv) {
  ...
  int start_idx =
      rank * cities_per_rank + (rank < remainder ? rank : remainder);
  ...
  MPI_Reduce(&local_served, &global_served, 1, MPI_FLOAT, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&local_demand, &global_demand, 1, MPI_FLOAT, MPI_SUM, 0,
             MPI_COMM_WORLD);
